...
const SelectCategory: React.FC<SelectCategoryProps> = ({ categories, onCategorySelect, selectedCategory }) => {
      ...
      <div className="md:hidden">
        <div className="relative w-full">
          ...
          >
            ...
            <ChevronDownIcon 
              className={`h-5 transition-transform duration-300 ${isRotated ? 'rotate-180' : ''}`}
            />
          </button>
          <div
            className={`flex flex-col gap-5 absolute w-full mt-2 bg-white border rounded-[2rem] p-5 transition-all duration-300 ease-in-out transform ${
              isOpen ? 'max-h-auto opacity-100 translate-y-0' : 'max-h-auto opacity-0 translate-y-[-20px] overflow-hidden'
            }`}
          >
            {categories.map((category) => (
                <div
                key={category}
                className={`cursor-pointer bodyMD ${selectedCategory === category ? 'font-bold' : ''}`}
