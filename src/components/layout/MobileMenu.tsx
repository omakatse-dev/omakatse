...
export default function MobileMenu({ isOpen, setIsOpen }: MobileMenuProps) {
    ...
    return (
        <div className={`fixed xl:hidden w-screen h-screen bg-yellow inset-0 z-10 p-8 transition-all overflow-auto ${!isOpen ? "-translate-y-full" : "-translate-y-0"}`}>

        <div className="flex justify-between pb-8 border-b-1 border-white">
        <button onClick={() => setIsOpen((prev) => !prev)}>
            <XMarkIcon className="h-6 w-6 stroke-black stroke-[2]" />
        </button>
        <Link onClick={() => setIsOpen((prev) => !prev)} href="/" className="flex gap-1 items-center">
            <Image
        ...
        <Link onClick={() => setIsOpen((prev) => !prev)} href="/shop" className="flex gap-1 items-center">
            Shop
            <ChevronDownIcon className="h-6 w-6 stroke-black stroke-[2]" />
        </Link>
        {links.map((link) => (
        <Link onClick={() => setIsOpen((prev) => !prev)} href={link.url} key={link.name}>
            {link.name}
        </Link>
        ))}
        </div>
        ...
        <Link onClick={() => setIsOpen((prev) => !prev)} href="/shop" className="flex gap-3 items-center">
